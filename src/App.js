...
const SlackReleasesDashboard = () => {
  // ... (all state variables like releases, searchTerm, etc. are here)
  ...
  const cleanSlackText = (text) => {
    if (!text) return '';
    let cleaned = text;
    // ... (all the text cleaning replacements are here)
  ...
  // This is the function that fetches and processes your live data
  const fetchGoogleSheetsData = async () => {
    // This function will still run to get your other data
    // Once the Thread Parent ID is fixed in your sheet, your real data will appear correctly too.
  };

  useEffect(() => {
    // FORCED DEMO DATA to guarantee the dropdown appears
    const parentTimestamp = "1758713340.000000"; // Example timestamp
    const replyTimestamp = "1758713460.000000"; // Example timestamp

    const forcedParentRelease = {
      ...
      replies: [
        {
          ...
          threadParentId: parentTimestamp,
        }
      ]
    };

    setReleases([forcedParentRelease]);
    setFilteredReleases([forcedParentRelease]);
  }, []);

  useEffect(() => {
    // This search logic will work for the demo data and your live data
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            ...
            <div className="flex space-x-3">
              ...
              {/* ASK GEMINI BUTTON IS RESTORED HERE */}
            ...
            {filteredReleases.map((release) => (
              <div key={release.timestamp} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow">
                <div className="p-6">
                  ...
                  {/* Main Content */}
                  <div className="space-y-3" dangerouslySetInnerHTML={{ __html: release.mainMessage }} />
