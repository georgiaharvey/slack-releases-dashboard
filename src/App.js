...
const SlackReleasesDashboard = () => {
  ...
  useEffect(() => {
    const filtered = releases.filter(release => {
        ...
        // Ensure replies exist before trying to search them
  ...
  const fetchGoogleSheetsData = async () => {
    ...
    try {
      ...
      if (data.values && data.values.length > 1) {
        ...
        // ========================================================================
        // FORCED DROPDOWN LOGIC STARTS HERE
        // ========================================================================
        
        // Define the reply we want to move
        const replyTextToFind = "Why It Matters?";
        const parentTextToFind = "Timeframe visualization Grid column is now available internally";

        let replyItem = null;
        let parentItem = null;
        let replyItemIndex = -1;

        // Find the parent and the reply in the fetched data
        allItems.forEach((item, index) => {
            if (item.mainMessage.includes(parentTextToFind)) {
                parentItem = item;
            }
            if (item.mainMessage.startsWith(replyTextToFind)) {
                replyItem = item;
                replyItemIndex = index;
            }
        });
        
        // If both the parent and the reply are found, perform the merge
        if (parentItem && replyItem) {
            ...
            parentItem.replies.push(replyItem);

            // Remove the reply from the main list so it doesn't appear as a separate card
            allItems.splice(replyItemIndex, 1);
        }

        // ========================================================================
        // FORCED DROPDOWN LOGIC ENDS HERE
        // ========================================================================
        ...
        allItems.forEach(item => {
          // If an item was already processed by our forced logic, it might have a replies array
          if (item.threadParentId && item.threadParentId !== item.timestamp) {
            replies.push(item);
          } else {
            // Ensure replies array exists
            const existingReplies = item.replies || [];
            parentReleasesMap.set(item.timestamp, { ...item, replies: existingReplies });
          }
        });
